...
class PipelineOrchestrator {
    ...
    /**
     * Execute URL parsing (direct web scraping without AI/Google Maps)
     * @param {Object} params - URL parsing parameters
     * @param {string} params.websiteUrl - URL to scrape
     * @param {string} params.taskName - Name of the parsing task
     * @returns {Promise<Array>} Scraping results
     */
    async executeUrlParsing(params) {
        if (this.isRunning) {
            throw new Error('Pipeline is already running');
        }
        ...
        try {
            this.isRunning = true;
            const { websiteUrl, taskName } = params;

            console.log('🌐 Starting URL parsing:', { websiteUrl, taskName });
            ...
            // Execute web scraping directly (pass just URL)
            const scrapingResults = await this.apifyClient.scrapeWebsiteDetails([websiteUrl]);

            console.log('🌐 Web scraping results:', scrapingResults);
            ...
            this.updateProgress('error', 0, 3, `Ошибка: ${error.message}`);
            throw error;
        } finally {
            this.isRunning = false;
        }
