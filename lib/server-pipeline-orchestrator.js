...
class ServerPipelineOrchestrator {
    ...
    async executeSearch(taskId) {
        ...
        try {
            ...
            const { task_name: taskName, search_query: searchQuery, website_url: websiteUrl, task_type: taskType, progress } = task;
            ...
            // Get resultsPerQuery from progress.config, with fallback to default
            const resultsPerQuery = progress?.config?.resultsPerQuery || 50;

            // Validate range (10-100)
            const resultCount = Math.max(10, Math.min(100, resultsPerQuery));
            ...
            const relevantResults = this.filterByRelevance(resultsWithContact, searchQuery);
    ...
    async scrapeOrganizationDetails(validatedResults) {
        try {
            ...
            try {
                ...
                // 🔍 DEBUG: Check allEmails IMMEDIATELY after scraping
                console.log('🔍 DEBUG: First 3 scraped results with allEmails:');
                scrapedData.slice(0, 3).forEach((result, idx) => {
                    console.log(`  [${idx}] ${result.organizationName || result.title || 'Unknown'}:`);
            ...
            // 🔍 DEBUG: Check allEmails AFTER merge
            console.log('🔍 DEBUG: First 3 merged results with allEmails:');
            mergedResults.slice(0, 3).forEach((result, idx) => {
                console.log(`  [${idx}] ${result.organizationName || result.name || 'Unknown'}:`);
                console.log(`      email: ${result.email}`);
                console.log(`      allEmails: ${JSON.stringify(result.allEmails)}`);
                console.log(`      allEmails type: ${typeof result.allEmails}`);
                console.log(`      allEmails length: ${result.allEmails?.length || 0}`);
     ...
     */
    filterByRelevance(results, originalQuery) {
        console.log('='.repeat(80));
        ...
        // REMOVED: No result count limit - return all scored results
        console.log(`✅ Отсортировано по релевантности: ${scoredResults.length} результатов (без обрезки)`);
        console.log(`💡 Все результаты с email возвращаются, отсортированные по релевантности`);
        console.log('='.repeat(80));
