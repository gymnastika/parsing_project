...
class ServerPipelineOrchestrator {
    ...
    async executeSearch(taskId) {
        ...
        try {
            ...
            console.log(`📊 Starting pipeline for task: ${task.task_name}`);
            
            // Mark task as running
            await this.tasksService.markAsRunning(taskId);
            await this.updateProgress(taskId, 'initializing', 0, 7, 'Запуск парсинга...');

            const { task_name: taskName, search_query: searchQuery, website_url: websiteUrl } = task;
            ...
            await this.updateProgress(taskId, 'filtering', 5, 7, `Фильтрация ${detailedResults.length} результатов по контактам (email/телефон)...`);
