...
class ServerApifyClient {
    ...
    /**
     * Scrape website details using parallel Apify runs for faster processing
     * @param {Array} urls - Array of URLs to scrape
     * @param {number} parallelRuns - Number of parallel Apify runs to launch (default: 10)
     * @returns {Promise<Array>} - Array of scraped data from all runs
     */
    async scrapeWebsiteDetailsParallel(urls, parallelRuns = 10) {
        ...
        try {
            console.log(`🚀 Starting PARALLEL website scraping for ${urls.length} URLs with ${parallelRuns} concurrent runs`);

            // Split URLs into chunks for parallel processing
            const urlChunks = this.chunkArray(urls, Math.ceil(urls.length / parallelRuns));
            const actualRunCount = urlChunks.length; // Might be less than parallelRuns if few URLs

            console.log(`📦 Split into ${actualRunCount} chunks (${urlChunks.map(c => c.length).join(', ')} URLs each)`);

            // Launch all runs in parallel
            const runPromises = urlChunks.map(async (urlChunk, index) => {
                try {
                    console.log(`🌐 [Run ${index + 1}/${actualRunCount}] Starting scraping for ${urlChunk.length} URLs...`);

                    // Use existing scrapeWebsiteDetails for each chunk
                    const results = await this.scrapeWebsiteDetails(urlChunk);

                    console.log(`✅ [Run ${index + 1}/${actualRunCount}] Completed: ${results.length} results`);
                    return results;
                } catch (error) {
                    console.error(`❌ [Run ${index + 1}/${actualRunCount}] Failed:`, error.message);
                    // Return empty array on error to not break Promise.all
                    return [];
                }
            });

            // Wait for all runs to complete
            console.log(`⏳ Waiting for ${actualRunCount} parallel runs to complete...`);
            const allResults = await Promise.all(runPromises);

            // Flatten results from all runs
            const combinedResults = allResults.flat();

            console.log(`🎉 PARALLEL scraping completed: ${combinedResults.length} total results from ${actualRunCount} runs`);
            console.log(`  - Average per run: ${(combinedResults.length / actualRunCount).toFixed(1)} results`);
            console.log(`  - With email: ${combinedResults.filter(r => r.email).length}`);
            console.log(`  - Without email: ${combinedResults.filter(r => !r.email).length}`);

            return combinedResults;

        } catch (error) {
            console.error(`❌ Parallel web scraping failed: ${error.message}`);
            throw error;
        }
    }
