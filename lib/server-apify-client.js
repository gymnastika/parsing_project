...
class ServerApifyClient {
    ...
    async detectPlanType() {
        try {
            ...
            if (!response.ok) {
                ...
                this.planInfo = { type: 'unknown', maxConcurrent: 1 };
                return;
            }
            ...
            const monthlyUsage = user.monthlyUsage?.ACTOR_COMPUTE_UNITS || 0;
            const limits = user.limits || {};
            
            if (limits.ACTOR_COMPUTE_UNITS_MONTHLY > 1000000) {
                this.planInfo = { type: 'paid', maxConcurrent: 10 };
                console.log('📊 Apify Plan Detected: PAID');
            } else {
                this.planInfo = { type: 'free', maxConcurrent: 1 };
                console.log('📊 Apify Plan Detected: FREE');
            }
    ...
    async searchGoogleMaps(params) {
        ...
        try {
            ...
            // Configure memory based on plan type
            const memoryMbytes = this.planInfo?.type === 'paid' ? 2048 : 1024;
            const timeoutSecs = this.planInfo?.type === 'paid' ? 300 : 180;
