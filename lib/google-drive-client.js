...
class GoogleDriveClient {
    ...
    async getAccessToken(userId) {
        try {
            ...
            // Use getValidAccessToken which proactively refreshes tokens 5 minutes before expiration
            // This prevents 403 errors from expired tokens during upload
            this.log('🔑 Getting valid access token with proactive refresh...');
            this.accessToken = await window.googleOAuth.getValidAccessToken(userId);
    ...
    async validateToken() {
        try {
            ...
            // Log token info with explicit scope details
            ...
            if (!hasDriveScope) {
                console.error('❌ Missing Google Drive scope in token!');
                console.error('  - Token has scopes:', tokenInfo.scope);
                console.error('  - Required scope: https://www.googleapis.com/auth/drive.file');
                throw new Error('Google Drive access not granted. Please reconnect your Google account with Drive permissions.');
            }

            console.log('✅ Token validation successful - Drive access confirmed');
            console.log('  - drive.file scope: PRESENT');
            return true;
    ...
    async realTimeUpload(file, metadata, uploadInfo) {
        try {
            ...
            // TEMPORARY FIX: Use multipart for files up to 100MB to avoid 403 error in resumable upload
            // TODO: Fix resumable upload 403 error and revert to 5MB threshold
            if (file.size > 100 * 1024 * 1024) { // > 100MB - use resumable
                console.warn('⚠️ File larger than 100MB, attempting resumable upload (may fail with 403)...');
                return this.resumableUploadWithProgress(file, metadata, uploadInfo);
            } else {
                console.log('📤 Using multipart upload for file:', this.formatFileSize(file.size));
                    ...
                    xhr.onload = () => {
                        if (xhr.status === 200 || xhr.status === 201) {
                            ...
                            try {
                                ...
                                console.error('🔴 Google Drive multipart upload error response:', errorBody);
                            } catch (e) {
                                errorDetails = xhr.responseText || 'No error details available';
                                console.error('🔴 Google Drive multipart upload error (text):', errorDetails);
                            }

                            console.error('🔴 Multipart upload failed:');
                            console.error('  - Status:', xhr.status, xhr.statusText);
                            console.error('  - URL: https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart');
                            console.error('  - Token (first 20 chars):', this.accessToken?.substring(0, 20) + '...');
    ...
    async initiateResumableSession(file, metadata) {
        ...
        if (!response.ok) {
            ...
            try {
                ...
                console.error('🔴 Google Drive API error (text):', errorDetails);
            }

            console.error('🔴 Upload initiation failed:');
            console.error('  - Status:', response.status, response.statusText);
            console.error('  - URL:', 'https://www.googleapis.com/upload/drive/v3/files?uploadType=resumable');
            console.error('  - Token (first 20 chars):', this.accessToken?.substring(0, 20) + '...');
            console.error('  - Error details:', errorDetails);

            throw new Error(`Failed to initiate resumable upload: ${response.status} - ${errorDetails}`);
        }
