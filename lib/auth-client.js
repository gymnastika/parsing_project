...
class SupabaseAuthClient {
    ...
    /**
     * Register a new user with email proxy (no confirmation needed)
     * @param {Object} userData - User registration data
     * @param {string} userData.email - Generated email from username
    ...
    async register(userData) {
        ...
        const { email, password, firstName, lastName, username, secretCode } = userData;
        ...
        try {
            ...
            // Register user with Supabase Auth using email proxy (no confirmation)
            ...
            if (data.session) {
                console.log('🚪 Signing out after registration to prevent auto-login...');
            ...
            return {
                user: data.user,
                session: null, // Always null after signOut
                needsConfirmation: true // Always require manual login
            };
     ...
     * Note: Database trigger may have already created an empty profile,
     * so we use UPDATE to ensure all fields are populated
     */
    async createUserProfile({ id, username, firstName, lastName }) {
        try {
            console.log('📝 Creating/updating profile for user:', { id, username });

            // First, try to update existing profile (created by database trigger)
    ...
    async login(username, password) {
        ...
        try {
            ...
            // Use email proxy to authenticate with Supabase
    ...
    async saveTelegramConnection(botToken, botName, chatId) {
        ...
        try {
            console.log('💾 Saving Telegram integration to database...');
            ...
            if (existing) {
                ...
                result = data;
                console.log('✅ Telegram integration updated');
            } else {
    ...
    async getTelegramConnection() {
        ...
        try {
            console.log('📥 Loading Telegram integration from database...');

            const { data, error } = await this.client
                .from('telegram_integrations')
                .select('bot_token, bot_name, chat_id, is_active, last_verified_at')
                .eq('user_id', this.currentUser.id)
                .maybeSingle();
            ...
            // Return connection data if exists and is active
            if (data && data.is_active) {
                console.log('✅ Telegram integration loaded successfully');
                return {
                    token: data.bot_token,
                    botName: data.bot_name,
                    chatId: data.chat_id,
                    connectedAt: data.last_verified_at
                };
            }

            console.log('ℹ️ No active Telegram integration found');
            return null;
    ...
    async removeTelegramConnection() {
        ...
        try {
            console.log('🗑️ Removing Telegram integration...');
            ...
            if (error) {
                console.error('❌ Telegram integration remove error:', error);
                throw new Error(`Ошибка удаления подключения Telegram: ${error.message}`);
            }

            console.log('✅ Telegram integration removed successfully');
            return data;
