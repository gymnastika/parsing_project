...
function ensureDirectories() {
    ...
    try {
        ...
        // Check if we're in Railway environment (production) or localhost (development)
        const isRailway = process.env.RAILWAY_ENVIRONMENT || process.env.RAILWAY_PROJECT_ID;

        if (!isRailway && VOLUME_PATH === '/app/data') {
            // Localhost environment - use local directory instead
            const localVolumePath = path.join(process.cwd(), 'data');
            VOLUME_CONFIG.VOLUME_PATH = localVolumePath;
            console.log(`🏠 Localhost detected - using local directory: ${localVolumePath}`);
        }

        const volumePath = VOLUME_CONFIG.VOLUME_PATH;

        // Ensure main volume directory exists
        if (!fs.existsSync(volumePath)) {
            fs.mkdirSync(volumePath, { recursive: true });
            console.log(`✅ Created main volume directory: ${volumePath}`);
        }

        // Create each required subdirectory
        directories.forEach(dir => {
            const fullPath = path.join(volumePath, dir);
        ...
        console.log('✅ Volume storage ready for persistent storage');
        return true;

    } catch (error) {
        console.error('❌ Volume initialization failed:', error);
