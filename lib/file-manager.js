...
class FileManager {
        ...
        try {
            ...
            try {
                ...
                if (file.size <= this.config.standardUploadLimit) {
                    uploadResult = await this.standardUpload(file, bucket, filePath);
                } else {
                    uploadResult = await this.resumableUpload(file, bucket, filePath, uploadId);
                }
            } catch (supabaseError) {
                ...
                uploadResult = await this.localFallbackUpload(file, filePath);
     ...
     */
    async standardUpload(file, bucket, filePath) {
     ...
     */
    async resumableUpload(file, bucket, filePath, uploadId) {
        ...
        return this.standardUpload(file, bucket, filePath);
    }
