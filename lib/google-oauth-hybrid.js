...
class GoogleOAuthHybrid {
    ...
    createMimeMessage(config) {
        ...
        const validEmails = to.filter(email => {
            const isValid = typeof email === 'string' && email.trim().length > 0 && email.includes('@');
            if (!isValid) {
                console.warn('⚠️ Invalid email in To header:', email);
            }
            return isValid;
        });

        if (validEmails.length === 0) {
            throw new Error('No valid email addresses in recipients list');
        }

        console.log('📧 Creating MIME message for recipients:', {
            total: to.length,
            valid: validEmails.length,
            sample: validEmails.slice(0, 3)
        });
