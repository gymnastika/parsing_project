...
class ApifyClient {
    ...
    detectPaidPlanIndicators(userInfo) {
        console.log('🔍 Analyzing plan indicators:', {
            plan: userInfo.plan,
            monthlyUsageUsd: userInfo.monthlyUsageUsd,
            proxy: userInfo.proxy
        });

        // Check various indicators of paid plan
        if (userInfo.plan && userInfo.plan !== 'FREE') {
            console.log('✅ PAID plan detected via plan field:', userInfo.plan);
            return true;
        }

        if (userInfo.monthlyUsageUsd && userInfo.monthlyUsageUsd > 0) {
            console.log('✅ PAID plan detected via usage:', userInfo.monthlyUsageUsd);
            return true;
        }
