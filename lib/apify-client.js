...
class ApifyClient {
    ...
    async detectPlanType() {
        try {
            ...
            console.warn('⚠️ Could not detect plan type, using safe defaults:', error.message);
            this.planInfo = {
                type: 'free',
                maxMemoryMB: 1024,
                maxConcurrent: 1,
                bufferMultiplier: 8 // УВЕЛИЧЕНО: 6 → 8 для соответствия основной логике
            };
    ...
    /**
     * Analyze plan type from user info
     */
    analyzePlanType(userInfo) {
        // Default to free plan assumptions
        let planType = {
            type: 'free',
            maxMemoryMB: 1024,
            maxConcurrent: 1,
            bufferMultiplier: 8, // УВЕЛИЧЕНО: 6 → 8 для получения больше результатов
            timeoutSecs: 180
        };

        // Detect paid plan indicators
        const isPaidPlan = this.detectPaidPlanIndicators(userInfo);

        if (isPaidPlan) {
            // Use actual limits from user plan
            const maxConcurrent = userInfo.plan?.maxConcurrentActorRuns || 3;
            const maxMemory = (userInfo.plan?.maxActorMemoryGbytes || 4) * 1024; // GB to MB

            planType = {
                type: 'paid',
                maxMemoryMB: Math.min(maxMemory, 4096), // Cap at 4GB for safety
                maxConcurrent: Math.min(maxConcurrent, 5), // Use up to 5 concurrent
                bufferMultiplier: 12,
                timeoutSecs: 600
            };
    ...
    detectPaidPlanIndicators(userInfo) {
        console.log('🔍 Analyzing plan indicators:', {
        ...
        // Check isPaying flag (most reliable)
        if (userInfo.isPaying === true) {
            console.log('✅ PAID plan detected via isPaying flag');
            return true;
        }

        // Check plan.id (plan is an object, not a string!)
        if (userInfo.plan && userInfo.plan.id && userInfo.plan.id !== 'FREE') {
            console.log('✅ PAID plan detected via plan.id:', userInfo.plan.id);
            return true;
        }

        // Check if monthly price > 0
        if (userInfo.plan && userInfo.plan.monthlyBasePriceUsd > 0) {
            console.log('✅ PAID plan detected via monthly price:', userInfo.plan.monthlyBasePriceUsd);
            return true;
        }

        // Check if user has higher limits
        if (userInfo.limits && userInfo.limits.monthlyUsageCredits > 100) {
            return true;
        }
    ...
    async searchGoogleMaps(params) {
        ...
        try {
            ...
            const hasMemory = await this.checkMemoryAvailability(1024); // Check for 1GB
            if (!hasMemory) {
                console.log('🧹 Insufficient memory detected, attempting cleanup...');
                const aborted = await this.abortAllRunningTasks();
                console.log(`🔧 Cleanup result: ${aborted} tasks aborted`);
                
                // Wait a bit for cleanup to take effect
                await this.delay(3000);
                
                // Check again after cleanup
                const hasMemoryAfterCleanup = await this.checkMemoryAvailability(1024);
                if (!hasMemoryAfterCleanup) {
                    throw new Error('Insufficient memory available even after cleanup. Please wait for running tasks to complete or upgrade to a paid plan.');
                }
            }
            ...
            const input = {
                ...
                // Memory optimization - use smaller memory allocation
                memoryMbytes: 1024, // Use 1GB instead of default 4GB
                timeoutSecs: 180 // 3 minute timeout to prevent hanging
            };
